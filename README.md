...
В отдельном терминале запустите обработчик фоновых задач:

```bash
make run-worker
```

По умолчанию Makefile читает `.env`. Другой файл можно передать через переменную `ENV_FILE`, например `make run-server ENV_FILE=.env.local`.
